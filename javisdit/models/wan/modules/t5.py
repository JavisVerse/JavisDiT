...
def _t5(name,
    ...
    # init model
    with torch.device(device if init_to_device else 'cpu'):
        model = model_cls(**kwargs)

    # set device
    model = model.to(dtype=dtype)
    if init_to_device:  # TODO: maybe unused
...
class T5EncoderModel:
    ...
    def encode(self, text):
        ...
        return dict(y=context, mask=mask)
