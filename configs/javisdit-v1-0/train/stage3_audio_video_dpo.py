...
dpo_enabled = True
# dpo_beta = 1000
dpo_beta = {"audio": 1000, "video": 3000}
...
bucket_config = {  # 7.5s/it, randomly assigning raw videos to pre-defined and proper buckets
    "240p": {65: (1.0, 5), 81: ((1.0, 0.5), 4)},
    "360p": {65: (1.0, 2), 81: ((1.0, 0.5), 2)},
    "480p": {65: (1.0, 1), 81: ((1.0, 0.5), 1)},
}
...
# Model settings
weight_path = "./outputs/stage2_av_sft"

model = dict(
    type="Wan2_1_T2V_1_3B",
    weight_init_from=weight_path,
    model_type='t2av',
    patch_size=(1, 2, 2),
    dim=1536,
    ffn_dim=8960,
...
lora_pretrained_dir = weight_path + '/lora'
## NOTE: otherwise, use `lora_pretrained_path` to only load the lora weights
# lora_pretrained_path = weight_path + '/lora/adapter_model.bin'
